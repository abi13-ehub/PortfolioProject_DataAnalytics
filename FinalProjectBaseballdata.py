...
# using list comprehension
    return [i for i in statistics if i[yearid] == str(year)]
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    # # sort on res
    # # sort is done using formula
    # res.sort(key=lambda x: x[1], reverse=True)
    # return res[:numplayers]
# res[:numplayers] is used because only a certain number of players are to be returned.
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    # statistics dict is created with batting statistics info as we need it for formula
    statistics = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])

    stat_for_year = filter_by_year(statistics, year, info['yearid'])
    top_player_list = top_player_ids(info, stat_for_year, formula, numplayers)
    player_names = lookup_player_names(info,top_player_list)
    return player_names
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    agg_players = {}
    for stat in statistics:
        if stat[playerid] in agg_players:
            for field in fields:
                if field in agg_players[stat[playerid]]:
                    agg_players[stat[playerid]][field] += int(stat(field))

        else:
            temp_dic = {}
            for field in fields:
                temp_dic[playerid] = stat[playerid]
                temp_dic[field] = int(stat[field])
            agg_players[stat[playerid]] = temp_dic
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    # statistics = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    # res =[]
    # stats = aggregate_by_player_id(statistics, info['playerid'], info['battingfields'])
    # top_player_list = top_player_ids(info, stats, formula, numplayers)
    # player_names = lookup_player_names(info, top_player_list)
    # return res.append(player_names)
...
def test_baseball_statistics():
    ...
    #  This one computes on base plus slugging percentage
